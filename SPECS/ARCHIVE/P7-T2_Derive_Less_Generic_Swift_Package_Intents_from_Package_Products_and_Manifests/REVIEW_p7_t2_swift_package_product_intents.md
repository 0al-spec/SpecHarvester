...
- `PYTHONPATH=src python -m pytest tests/test_collector.py -q` - PASS, 52 passed.
- `PYTHONPATH=src python -m pytest` - PASS, 129 passed.
- `ruff check src tests` - PASS.
- `ruff format --check src tests` - PASS.
- `PYTHONPATH=src python -m pytest --cov=spec_harvester --cov-report=term-missing --cov-fail-under=90` - PASS, 90.15% total coverage.
