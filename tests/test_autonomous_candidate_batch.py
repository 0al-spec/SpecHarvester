...
def test_autonomous_candidate_batch_rejects_boolean_trusted_run_count(
    tmp_path: Path,
) -> None:
    inputs = write_source_manifest(tmp_path)
    run_report = write_trusted_local_adapter_run_report_fixture(tmp_path)
    payload = json.loads(run_report.read_text(encoding="utf-8"))
    payload["runner"]["executedAdapterCount"] = False
    run_report.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    output = tmp_path / "output"
    ...
    try:
        ...
        assert "integer runner.executedAdapterCount" in str(exc)
    else:
        raise AssertionError("expected boolean trusted run count to fail")
