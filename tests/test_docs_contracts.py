...
def assert_current_next_task(next_text: str) -> None:
    ...
    if (
        ...
        assert_p42_t9_recent(next_text)
        assert_phase_42_t10_planned(next_text)
        return
    ...
    if "# Next Task: P42-T4 Disabled Trusted Local Adapter Sandbox Runner Validation" in (
        ...
        assert_p42_t3_last_archived(next_text)
        assert_p42_t3_recent(next_text)
        assert_phase_42_t4_planned(next_text)
        return

    if "# Next Task: P42-T3 Trusted Local Adapter Sandbox Preflight Report Fixture" in next_text:
        assert_p42_t2_last_archived(next_text)
        assert_p42_t2_recent(next_text)
        assert_phase_42_t3_planned(next_text)
        return
    ...
    if "# Next Task: P41-T5 Trusted Local Adapter Run Evidence Handoff" in next_text:
        ...
        assert_phase_41_t5_planned(next_text)
        return
    ...
    if "# Next Task: P41-T3 Trusted Local Adapter Run Preflight Report Fixture" in next_text:
        ...
        assert_phase_41_t3_planned(next_text)
        return
    ...
    if "# Next Task: P40-T5 Adapter Evidence Batch Integration" in next_text:
        ...
        assert_phase_40_t5_planned(next_text)
        return
    ...
    if "# Next Task: P40-T3 Repository Plugin Adapter Preflight Report Fixture" in next_text:
        ...
        assert_phase_40_t3_planned(next_text)
        return
    ...
    if "# Next Task: P38-T4 Repository Plugin Batch Integration" in next_text:
        ...
        assert_phase_38_t4_planned(next_text)
        return
...
def assert_p38_t3_last_archived(next_text: str) -> None:
    assert "**Last Archived:** P38-T3 Repository Plugin Applicability Report Fixture" in next_text


def assert_p38_t3_recent(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert (
        "`P38-T3` added the machine-readable repository plugin applicability report fixture"
        in normalized
    )
    assert "generic-applicability-report.example.json" in next_text
    assert "SpecHarvesterRepositoryPluginApplicabilityReport" in next_text
    assert "producer_plugin_applicability_only" in next_text
    assert "decisionAuthority" in next_text
    assert "pluginOutputAuthority" in next_text
    assert "selected, rejected, fallback, and blocked decisions" in normalized
    ...
    assert "static evidence" in normalized
    assert "sidecar producer evidence" in normalized
    assert "not execute plugins" in normalized
    assert "not run AI" in normalized
    assert "not accept packages" in normalized
    assert "not accept relations" in normalized
    assert "not publish registry metadata" in normalized
    assert "not remove `preview_only`" in normalized
    assert "not treat plugin decisions as registry truth" in normalized
...
def assert_phase_38_t4_planned(next_text: str) -> None:
    ...
    assert "autonomous candidate batch" in normalized
    assert "sidecar producer evidence" in normalized
    assert "reads registry and applicability fixtures" in normalized
    assert "preserving existing parser and repository profile behavior" in normalized
    assert "must not implement plugin execution" in normalized
    assert "must not run plugins" in normalized
    assert "must not load third-party code" in normalized
    assert "must not change parser profile behavior" in normalized
    assert "must not change repository profile scoring" in normalized
    ...
    assert "must not accept packages" in normalized
    assert "must not accept relations" in normalized
    assert "must not publish registry metadata" in normalized
    assert "must not remove `preview_only`" in normalized
...
def assert_p40_t2_last_archived(next_text: str) -> None:
    assert "**Last Archived:** P40-T2 Repository Plugin Adapter Manifest Fixture" in next_text


def assert_p40_t2_recent(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "`P40-T2` added the first machine-readable" in normalized
    assert "SpecHarvesterRepositoryPluginAdapterManifest" in next_text
    assert "tests/fixtures/repository_plugins/adapter-manifest.example.json" in next_text
    assert "docs/REPOSITORY_PLUGIN_ADAPTER_MANIFEST_FIXTURE.md" in next_text
    assert "RepositoryPluginAdapterManifestFixture.md" in next_text
    assert "adapter ids" in normalized
    assert "contract versions" in normalized
    assert "supported roles" in normalized
    assert "required and optional evidence kinds" in normalized
    ...
    assert "sandbox requirements" in normalized
    assert "capability requests" in normalized
    assert "mode: static_only" in normalized
    assert "defaultEnabled: false" in normalized
    assert "requiresOperatorOptIn: true" in normalized
    ...
    assert "adapterPreflight: not_run" in normalized
...
def assert_phase_40_t3_planned(next_text: str) -> None:
    ...
    assert "P40-T2 manifest fixture" in normalized
    assert "static evidence envelope" in normalized
    assert "allowed, rejected, fallback, and blocked adapter decisions" in normalized
    assert "Refuse unsafe execution" in normalized
    assert "missing required evidence" in normalized
    assert "without executing adapter code" in normalized
    assert "language- and framework-agnostic" in normalized
    assert "Do not implement adapter loading or execution" in normalized
    assert "Do not change static plugin applicability evaluation" in normalized
    assert "Do not change `autonomous-candidate-batch`" in normalized
...
def assert_p40_t4_last_archived(next_text: str) -> None:
    assert "**Last Archived:** P40-T4 Adapter Execution Policy" in next_text


def assert_p40_t4_recent(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "`P40-T4` documented the disabled-by-default" in normalized
    assert "docs/REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md" in next_text
    assert "RepositoryPluginAdapterExecutionPolicy.md" in next_text
    assert "disabled" in normalized
    assert "static_only" in normalized
    assert "trusted_local_tool" in normalized
...
def assert_phase_40_t5_planned(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "# Next Task: P40-T5 Adapter Evidence Batch Integration" in next_text
    assert "**Status:** Planned" in next_text or "**Status:** In Progress" in next_text
    assert "`feature/P40-T5-adapter-evidence-batch-integration`" in next_text
    assert "Phase 40. Repository Plugin Adapter Contract" in next_text
    assert "adapter manifest and preflight output" in normalized
    assert "`autonomous-candidate-batch`" in normalized
    assert "review-only producer evidence" in normalized
    assert "existing static evaluator path unchanged" in normalized
    assert "operator explicitly supplies adapter evidence" in normalized
    assert "Do not implement adapter loading or execution" in normalized
...
def assert_p41_t2_last_archived(next_text: str) -> None:
    assert "**Last Archived:** P41-T2 Trusted Local Adapter Run Request Fixture" in next_text


def assert_p41_t2_recent(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "`P41-T2` added the machine-readable" in normalized
    assert "SpecHarvesterTrustedLocalAdapterRunRequest" in normalized
    assert "trusted-local-adapter-run-request.example.json" in next_text
    assert "TRUSTED_LOCAL_ADAPTER_RUN_REQUEST_FIXTURE.md" in next_text
    assert "TrustedLocalAdapterRunRequestFixture.md" in next_text
    assert "adapter manifest/preflight references" in normalized
    assert "explicit operator opt-in" in normalized
    assert "safe relative read path allowlists" in normalized
...
def assert_phase_41_t3_planned(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "# Next Task: P41-T3 Trusted Local Adapter Run Preflight Report Fixture" in next_text
    assert "**Status:** Planned" in next_text or "**Status:** In Progress" in next_text
    assert "`feature/P41-T3-trusted-local-adapter-run-preflight-report-fixture`" in next_text
    assert "Phase 41. Trusted Local Adapter Runtime Readiness" in next_text
    assert "SpecHarvesterTrustedLocalAdapterRunPreflightReport" in normalized
    assert "SpecHarvesterTrustedLocalAdapterRunRequest" in normalized
    assert "explicit operator opt-in" in normalized
    assert "unsafe paths" in normalized
    assert "missing or mismatched digests" in normalized
    assert "undeclared input artifacts" in normalized
    assert "undeclared output paths" in normalized
    assert "network access" in normalized
    assert "dependency installation" in normalized
    assert "package manager invocation" in normalized
    assert "harvested code execution" in normalized
    assert "AI execution" in normalized
    assert "unbounded process execution" in normalized
    assert "preflight pass is not execution permission" in normalized
    assert "Do not implement adapter loading or execution" in normalized
    assert "Do not run adapter processes" in normalized
...
def assert_p41_t4_last_archived(next_text: str) -> None:
    assert "**Last Archived:** P41-T4 Disabled Trusted Local Adapter Runner Skeleton" in next_text


def assert_p41_t4_recent(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "`P41-T4` added the disabled" in normalized
    assert "trusted-local-adapter-runner-skeleton" in normalized
    assert "SpecHarvesterTrustedLocalAdapterRunReport" in normalized
    assert "TRUSTED_LOCAL_ADAPTER_RUNNER_SKELETON.md" in next_text
    assert "TrustedLocalAdapterRunnerSkeleton.md" in next_text
    assert "adapterExecution: not_run" in normalized
    assert "adapterCodeLoaded: false" in normalized
    assert "adapterProcessSpawned: false" in normalized
    assert "runnerReportIsExecutionPermission: false" in normalized
    assert "producer-side review evidence" in normalized


def assert_phase_41_t5_planned(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "# Next Task: P41-T5 Trusted Local Adapter Run Evidence Handoff" in next_text
    assert "**Status:** Planned" in next_text or "**Status:** In Progress" in next_text
    assert "`feature/P41-T5-trusted-local-adapter-run-evidence-handoff`" in next_text
    assert "Phase 41. Trusted Local Adapter Runtime Readiness" in next_text
    assert "Connect trusted local adapter run reports" in normalized
    assert "autonomous-candidate-batch" in normalized
    assert "review-only producer evidence" in normalized
    assert "default static evaluator" in normalized
    assert "existing adapter sidecar paths" in normalized
    assert "SpecHarvesterTrustedLocalAdapterRunReport" in normalized
    assert "adapterExecution: not_run" in normalized
    assert "adapterCodeLoaded: false" in normalized
    assert "Do not implement real adapter execution" in normalized
...
def assert_p42_t2_last_archived(next_text: str) -> None:
    assert "**Last Archived:** P42-T2 Trusted Local Adapter Sandbox Contract Fixture" in next_text


def assert_p42_t2_recent(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "`P42-T2` added the machine-readable" in normalized
    assert "SpecHarvesterTrustedLocalAdapterSandboxContract" in normalized
    assert "TRUSTED_LOCAL_ADAPTER_SANDBOX_CONTRACT_FIXTURE.md" in next_text
    assert "TrustedLocalAdapterSandboxContractFixture.md" in next_text
    assert "trusted-local-adapter-sandbox-contract.example.json" in normalized
    assert "adapterExecution: not_run" in normalized
    assert "adapterProcessSpawned: false" in normalized
    assert "registryAuthority: false" in normalized


def assert_phase_42_t3_planned(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "# Next Task: P42-T3 Trusted Local Adapter Sandbox Preflight Report Fixture" in next_text
    assert "**Status:** Planned" in next_text or "**Status:** In Progress" in next_text
    assert "`feature/P42-T3-trusted-local-adapter-sandbox-preflight-report-fixture`" in next_text
    assert "SpecHarvesterTrustedLocalAdapterSandboxPreflightReport" in normalized
    assert "SpecHarvesterTrustedLocalAdapterSandboxContract" in normalized
    assert "sandbox contract identity" in normalized
    assert "operator approval requirements" in normalized
    assert "filesystem/environment/network/dependency policy" in normalized
    assert "output verification" in normalized
    assert "audit requirements" in normalized
    assert "before any sandbox runner implementation" in normalized
    assert "Do not implement real adapter execution" in normalized
    assert "Do not spawn adapter processes" in normalized
    assert "Do not treat sandbox preflight as registry truth" in normalized


def assert_p42_t3_last_archived(next_text: str) -> None:
    assert (
...
def assert_p42_t3_recent(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "`P42-T3` added the machine-readable" in normalized
    assert "SpecHarvesterTrustedLocalAdapterSandboxPreflightReport" in normalized
    assert "TRUSTED_LOCAL_ADAPTER_SANDBOX_PREFLIGHT_REPORT_FIXTURE.md" in next_text
    assert "TrustedLocalAdapterSandboxPreflightReportFixture.md" in next_text
    assert "trusted-local-adapter-sandbox-preflight-report.example.json" in normalized
    assert "sandbox_preflight_passed_review_only" in normalized
    assert "adapterExecution: not_run" in normalized
    assert "adapterProcessSpawned: false" in normalized
    assert "registryAuthority: false" in normalized


def assert_phase_42_t4_planned(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert "# Next Task: P42-T4 Disabled Trusted Local Adapter Sandbox Runner Validation" in (
        next_text
    )
    assert "**Status:** Planned" in next_text or "**Status:** In Progress" in next_text
    assert "`feature/P42-T4-disabled-trusted-local-adapter-sandbox-runner-validation`" in (
        next_text
    )
    assert "disabled trusted local adapter sandbox runner validation" in normalized
    assert "SpecHarvesterTrustedLocalAdapterSandboxContract" in normalized
    assert "SpecHarvesterTrustedLocalAdapterSandboxPreflightReport" in normalized
    assert "contract/preflight linkage" in normalized
    assert "adapterExecution: not_run" in normalized
    assert "Do not load third-party adapter code" in normalized
    assert "Do not spawn adapter processes" in normalized
...
def assert_p42_t9_last_archived(next_text: str) -> None:
    assert (
        "**Last Archived:** P42-T9 Explicit Real Local Trusted Adapter Sandbox Run Request "
        "Preflight Fixture"
    ) in next_text


def assert_p42_t9_recent(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert (
        "`P42-T9` added "
    ...
    assert "TrustedLocalAdapterExplicitRealLocalSandboxRunRequestPreflightFixture.md" in next_text
    assert "SpecHarvesterExplicitRealLocalTrustedAdapterSandboxRunRequest" in normalized
    assert "verifies its SHA-256 digest" in normalized
    assert "P42-T6 verifier requirements" in normalized
    assert "P42-T7 readiness requirements" in normalized
    assert "scoped operator approval binding" in normalized
    assert "adapter package identity" in normalized
    assert "target repository identity" in normalized
    assert "sandbox policy identity" in normalized
    assert "runtime policy" in normalized
    ...
    assert "accepted, rejected, blocked, and warning checks" in normalized
    assert "unsafe paths" in normalized
    assert "missing evidence requirements" in normalized
    assert "reusable approval" in normalized
    assert "execution permission drift" in normalized
    assert "adapter code loading" in normalized
    assert "adapter process spawning" in normalized
    assert "harvested-code execution" in normalized
    assert "AI execution" in normalized
    assert "registry authority" in normalized
    assert "adapter output acceptance" in normalized
    assert "preflightPassIsExecutionPermission: false" in normalized
    assert "preflightPassIsOperatorApproval: false" in normalized
    assert "preflightPassIsRegistryAuthority: false" in normalized
    assert "adapterExecution: not_run" in normalized
    assert "adapterCodeLoaded: false" in normalized
    assert "adapterProcessSpawned: false" in normalized
    assert "executedAdapterCount: 0" in normalized
...
def assert_phase_42_t10_planned(next_text: str) -> None:
    normalized = " ".join(next_text.split())
    assert (
        "# Next Task: P42-T10 Disabled Explicit Real Local Trusted Adapter Sandbox Runner Skeleton"
    ) in next_text
    ...
    assert "disabled explicit real local trusted adapter sandbox runner skeleton" in normalized
    assert "validates the P42-T8 request and P42-T9 preflight fixture linkage" in normalized
    assert "without loading adapter code or spawning adapter processes" in normalized
    assert "P42-T8 request identity and digest linkage" in normalized
    assert "P42-T9 preflight identity, digest linkage" in normalized
    assert "passed review-only result" in normalized
...
def test_repository_plugin_applicability_report_fixture_is_documented() -> None:
    fixture_path = (
        ROOT
        / "tests"
        / "fixtures"
        / "repository_plugins"
        / "generic-applicability-report.example.json"
    )
    registry_path = (
        ROOT / "tests" / "fixtures" / "repository_plugins" / "generic-registry.example.json"
    )
    github_doc = ROOT / "docs" / "REPOSITORY_PLUGIN_APPLICABILITY_REPORT_FIXTURE.md"
    docc_doc = (
        ROOT
        / "Sources"
        / "SpecHarvester"
    ...
    selected = {decision["pluginId"]: decision for decision in payload["selectedPlugins"]}
    ...
    decisions = [
        ...
        *payload["rejectedPlugins"],
        *payload["fallbackPlugins"],
    ...
    for decision in decisions:
        ...
        assert decision["decisionAuthority"] == payload["authority"]
        assert decision["decisionAuthority"] == "producer_plugin_applicability_only"
        assert decision["pluginOutputAuthority"] == registry_plugin["authority"]
        assert decision["pluginOutputAuthority"] == "producer_side_evidence_only"
        assert decision["decision"] in {"selected", "rejected", "fallback", "blocked"}
        assert decision["confidence"] in {"high", "medium", "low", "blocked"}
        assert decision["reasonCodes"]
        assert set(decision["evidencePaths"]).issubset(static_paths)
    ...
    assert fallback["spec_harvester.generic.parser_profile.v0"]["reasonCodes"] == [
        "no_specific_parser_profile_selected",
    ...
    assert len(diagnostics) == payload["summary"]["diagnosticCount"]
    ...
    assert "REPOSITORY_PLUGIN_APPLICABILITY_REPORT_FIXTURE.md" in docs_index.read_text(
        encoding="utf-8"
    )
    assert "docs/REPOSITORY_PLUGIN_APPLICABILITY_REPORT_FIXTURE.md" in docc_root.read_text(
        encoding="utf-8"
    )
    assert "<doc:RepositoryPluginApplicabilityReportFixture>" in docc_root.read_text(
        encoding="utf-8"
    )
    assert "REPOSITORY_PLUGIN_APPLICABILITY_REPORT_FIXTURE.md" in capabilities.read_text(
        encoding="utf-8"
    )
    assert "RepositoryPluginApplicabilityReportFixture" in capabilities_docc.read_text(
        encoding="utf-8"
    )
    assert "REPOSITORY_PLUGIN_APPLICABILITY_REPORT_FIXTURE.md" in roadmap.read_text(
        encoding="utf-8"
    )
    assert "RepositoryPluginApplicabilityReportFixture" in roadmap_docc.read_text(encoding="utf-8")
    assert "REPOSITORY_PLUGIN_APPLICABILITY_REPORT_FIXTURE.md" in registry_doc.read_text(
        encoding="utf-8"
    )
    assert "RepositoryPluginApplicabilityReportFixture" in registry_docc.read_text(encoding="utf-8")
    assert "REPOSITORY_PLUGIN_APPLICABILITY_REPORT_FIXTURE.md" in subsystem_doc.read_text(
        encoding="utf-8"
    )
    assert "RepositoryPluginApplicabilityReportFixture" in subsystem_docc.read_text(
        encoding="utf-8"
    )
...
def test_repository_plugin_adapter_execution_policy_is_documented() -> None:
    github_doc = ROOT / "docs" / "REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md"
    docc_doc = (
        ROOT
        / "Sources"
        / "SpecHarvester"
    ...
    for path in (github_doc, docc_doc):
        ...
        for required in (
            ...
            "symlink escapes",
            "missing digests",
            "passing adapter preflight report",
            "read path allowlist",
        ...
        for boundary in (
            "does not implement adapter loading",
            "adapter execution",
            "sandbox launch",
            "batch integration",
            "registry publication",
            "does not enable adapter runtime",
        ):
            assert boundary in normalized, f"Boundary {boundary!r} not found in {path}"

    for path, required in (
        (docs_index, "REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md"),
        (docc_root, "docs/REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md"),
        (docc_root, "<doc:RepositoryPluginAdapterExecutionPolicy>"),
        (capabilities, "REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md"),
        (capabilities_docc, "RepositoryPluginAdapterExecutionPolicy"),
        (roadmap, "REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md"),
        (roadmap_docc, "RepositoryPluginAdapterExecutionPolicy"),
        (subsystem_doc, "REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md"),
        (subsystem_docc, "RepositoryPluginAdapterExecutionPolicy"),
        (adapter_contract, "REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md"),
        (adapter_contract_docc, "RepositoryPluginAdapterExecutionPolicy"),
        (manifest_doc, "REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md"),
        (manifest_docc, "RepositoryPluginAdapterExecutionPolicy"),
        (preflight_doc, "REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md"),
        (preflight_docc, "RepositoryPluginAdapterExecutionPolicy"),
    ):
        assert required in path.read_text(encoding="utf-8"), (
            f"Reference {required!r} not found in {path}"
        )
...
def test_trusted_local_adapter_sandbox_contract_fixture_is_documented() -> None:
    fixture = (
        ROOT
        / "tests"
        / "fixtures"
    ...
    github_doc = ROOT / "docs" / "TRUSTED_LOCAL_ADAPTER_SANDBOX_CONTRACT_FIXTURE.md"
    docc_doc = (
        ROOT
        / "Sources"
        / "SpecHarvester"
    ...
    sandbox_doc = ROOT / "docs" / "TRUSTED_LOCAL_ADAPTER_RUNTIME_SANDBOX_PLAN.md"
    sandbox_docc = (
        ROOT
        / "Sources"
        / "SpecHarvester"
    ...
    assert payload["apiVersion"] == "spec-harvester.trusted-local-adapter-sandbox-contract/v0"
    assert payload["kind"] == "SpecHarvesterTrustedLocalAdapterSandboxContract"
    assert payload["schemaVersion"] == 1
    assert payload["authority"] == "producer_trusted_local_adapter_sandbox_contract_only"
    assert payload["contract"]["sandboxContractIsExecutionPermission"] is False
    assert payload["contract"]["defaultExecution"] == "disabled"

    references = payload["inputContractReferences"]
    for reference in references.values():
        path = ROOT / reference["path"]
        digest = "sha256:" + hashlib.sha256(path.read_bytes()).hexdigest()
        assert reference["digest"] == digest
        assert reference["path"].startswith("tests/fixtures/repository_plugins/")
        assert ".." not in reference["path"]
        assert not reference["path"].startswith("/")
        assert "\\" not in reference["path"]

    declared_inputs = payload["declaredInputArtifacts"]
    assert {item["kind"] for item in declared_inputs} == {
        "trusted_local_adapter_run_request",
        "trusted_local_adapter_run_preflight_report",
        "adapter_manifest",
        "adapter_preflight_report",
        "repository_plugin_static_evidence_envelope",
        "repository_plugin_registry",
    }
    ...
    assert (
        adapter_identity["adapterId"] == "spec_harvester.adapters.generic.parser_profile_summary.v0"
    )
    assert adapter_identity["pinnedRevisionOrDigestRequired"] is True
    assert adapter_identity["mutableRefAllowed"] is False
    assert adapter_identity["signatureOrDigestVerification"] == "digest_required_before_runtime"
    assert adapter_identity["packageSource"]["digest"] == (
        "sha256:"
        + hashlib.sha256(
            (ROOT / adapter_identity["packageSource"]["path"]).read_bytes()
        ).hexdigest()
    )

    sandbox_policy = payload["sandboxPolicyIdentity"]
    assert sandbox_policy["policyId"] == "spec_harvester.trusted_local_adapter.local_sandbox.v0"
    assert sandbox_policy["sourceTask"] == "P42-T2"
    assert sandbox_policy["runtimeMustRecordPolicyDigest"] is True

    approval = payload["operatorApprovalRequirements"]
    assert approval["requiredBeforeRuntime"] is True
    assert approval["providedByFixture"] is False
    assert approval["approvalScope"] == "single_adapter_single_repository_single_run"
    assert approval["approvalIsRegistryAcceptance"] is False
    assert "adapterPackageIdentity.packageSource.digest" in approval["approvalMustBind"]

    process = payload["processPolicy"]
    assert process["adapterProcessExecution"] == "not_allowed_in_contract_fixture"
    assert process["futureRuntimeRequiresSandboxPreflight"] is True
    assert process["noAmbientCredentials"] is True
    assert process["resourceBudgets"]["currentMaxProcessCount"] == 0
    assert process["resourceBudgets"]["futureRuntimeMaxProcessCount"] == 1

    filesystem = payload["filesystemPolicy"]
    assert filesystem["pathFormat"] == "posix_relative"
    assert filesystem["parentSegmentsAllowed"] is False
    assert filesystem["absolutePathsAllowed"] is False
    ...
    assert environment["inheritEnvironment"] is False
    assert environment["allowedEnvironmentVariables"] == []
    assert environment["secretsAccess"] == "not_allowed"
    assert environment["modelProviderTokenAccess"] == "not_allowed"

    dependency = payload["dependencyPolicy"]
    assert dependency["dependencyInstallation"] == "not_allowed"
    assert dependency["packageManagers"] == "not_invoked"
    assert dependency["packageManagerCommands"] == []

    network = payload["networkPolicy"]
    assert network["networkAccess"] == "none"
    assert network["dnsAllowed"] is False
    assert network["remoteFetchAllowed"] is False
    assert network["targetAllowlist"] == []

    output = payload["outputVerification"]
    assert output["required"] is True
    assert output["sha256DigestRequired"] is True
    assert output["producerRunIdRequired"] is True
    assert output["adapterIdAndDigestRequired"] is True
    assert output["sourceInputDigestsRequired"] is True
    assert output["outputIsRegistryTruth"] is False

    audit = payload["auditRecordRequirements"]
    ...
    for field in (
        ...
        assert field in audit["requiredFields"]
    ...
    assert payload["followUp"]["sandboxPreflightReportTask"] == "P42-T3"
    ...
    for path, required in (
        (sandbox_doc, "TRUSTED_LOCAL_ADAPTER_SANDBOX_CONTRACT_FIXTURE.md"),
        (sandbox_docc, "TrustedLocalAdapterSandboxContractFixture"),
        (readiness_doc, "TRUSTED_LOCAL_ADAPTER_SANDBOX_CONTRACT_FIXTURE.md"),
        (readiness_docc, "TrustedLocalAdapterSandboxContractFixture"),
        (docs_index, "TRUSTED_LOCAL_ADAPTER_SANDBOX_CONTRACT_FIXTURE.md"),
        (docc_root, "TrustedLocalAdapterSandboxContractFixture"),
        (capabilities, "TRUSTED_LOCAL_ADAPTER_SANDBOX_CONTRACT_FIXTURE.md"),
        (capabilities_docc, "TrustedLocalAdapterSandboxContractFixture"),
        (roadmap, "TRUSTED_LOCAL_ADAPTER_SANDBOX_CONTRACT_FIXTURE.md"),
        (roadmap_docc, "TrustedLocalAdapterSandboxContractFixture"),
    ):
        assert required in path.read_text(encoding="utf-8"), (
            f"Reference {required!r} not found in {path}"
        )
    ...
    workplan_text = workplan.read_text(encoding="utf-8")
    assert "`P42-T2` Add a machine-readable" in workplan_text
    assert "`P42-T3` Add a trusted local adapter sandbox preflight report fixture" in (
        workplan_text
    )
    assert_current_next_task(next_task.read_text(encoding="utf-8"))


def test_trusted_local_adapter_sandbox_preflight_report_fixture_is_documented() -> None:
    fixture = (
        ROOT
        / "tests"
        / "fixtures"
    ...
    github_doc = ROOT / "docs" / "TRUSTED_LOCAL_ADAPTER_SANDBOX_PREFLIGHT_REPORT_FIXTURE.md"
    docc_doc = (
        ROOT
        / "Sources"
        / "SpecHarvester"
    ...
    assert payload["apiVersion"] == "spec-harvester.trusted-local-adapter-sandbox-preflight/v0"
    assert payload["kind"] == "SpecHarvesterTrustedLocalAdapterSandboxPreflightReport"
    assert payload["schemaVersion"] == 1
    assert payload["authority"] == "producer_trusted_local_adapter_sandbox_preflight_only"
    assert payload["contract"] == {
        "purpose": (
            "Validate a trusted local adapter sandbox contract before any sandbox runner "
            "implementation without granting execution authority."
        ),
        "contractVersion": "0.1.0",
        "sandboxContractAuthority": "producer_trusted_local_adapter_sandbox_contract_only",
        "sandboxPreflightAuthority": "producer_trusted_local_adapter_sandbox_preflight_only",
        "outputAuthority": "producer_adapter_output_candidate_evidence_only",
        "defaultExecution": "disabled",
        "preflightPassIsExecutionPermission": False,
    }

    contract_ref = payload["sandboxContract"]
    sandbox_contract_path = (
        "tests/fixtures/repository_plugins/trusted-local-adapter-sandbox-contract.example.json"
    )
    assert contract_ref == {
        "path": sandbox_contract_path,
    ...
    assert ".." not in contract_ref["path"]
    assert not contract_ref["path"].startswith("/")
    assert "\\" not in contract_ref["path"]
    ...
    assert payload["validatedSandboxContract"] == {
        "apiVersion": "spec-harvester.trusted-local-adapter-sandbox-contract/v0",
        "kind": "SpecHarvesterTrustedLocalAdapterSandboxContract",
        "schemaVersion": 1,
        "authority": "producer_trusted_local_adapter_sandbox_contract_only",
        "sandboxPolicyId": "spec_harvester.trusted_local_adapter.local_sandbox.v0",
        "operatorApprovalRequiredBeforeRuntime": True,
        "operatorApprovalProvidedByFixture": False,
        "sandboxContractIsExecutionPermission": False,
        "adapterExecution": "not_run",
        "registryAuthority": False,
    }
    ...
    accepted = {check["code"]: check for check in payload["acceptedChecks"]}
    assert set(accepted) == {
        "sandbox_contract_identity_valid",
        "sandbox_contract_digest_verified",
        "adapter_package_identity_digest_pinned",
        "sandbox_policy_identity_present",
        "operator_approval_requirements_present",
        "process_policy_bounded_and_no_execution",
        "filesystem_policy_safe_relative_declared_only",
        "environment_policy_sealed",
        "dependency_policy_denies_installation",
        "network_policy_denies_remote_access",
        "output_verification_requirements_present",
        "audit_requirements_replayable",
        "execution_boundary_review_only",
        "non_authority_statements_present",
    }
    assert all(check["status"] == "passed" for check in accepted.values())

    rejected = {check["code"]: check for check in payload["rejectedChecks"]}
    assert set(rejected) == {
        "unsafe_parent_path_rejected",
        "unsafe_absolute_path_rejected",
        "unsafe_backslash_path_rejected",
        "unsafe_network_path_rejected",
        "missing_sandbox_contract_digest_rejected",
        "sandbox_contract_digest_mismatch_rejected",
        "mutable_adapter_ref_rejected",
        "missing_operator_approval_requirement_rejected",
        "approval_provided_by_contract_fixture_rejected",
        "adapter_process_execution_allowed_rejected",
        "unbounded_process_tree_rejected",
        "environment_inheritance_rejected",
        "secrets_access_rejected",
    ...
    assert all(check["status"] == "rejected" for check in rejected.values())

    blocked = {check["code"]: check for check in payload["blockedChecks"]}
    assert set(blocked) == {
        "sandbox_runner_execution_blocked",
        "third_party_adapter_code_loading_blocked",
        "adapter_process_spawn_blocked",
        "synthetic_approved_adapter_run_blocked",
        "real_local_adapter_run_blocked",
        "registry_authority_requested_blocked",
    }
    assert all(check["status"] == "blocked" for check in blocked.values())
    ...
    assert payload["followUp"]["disabledSandboxRunnerValidationTask"] == "P42-T4"
    ...
    for path, required in (
        (contract_doc, "TRUSTED_LOCAL_ADAPTER_SANDBOX_PREFLIGHT_REPORT_FIXTURE.md"),
        (contract_docc, "TrustedLocalAdapterSandboxPreflightReportFixture"),
        (sandbox_plan, "TRUSTED_LOCAL_ADAPTER_SANDBOX_PREFLIGHT_REPORT_FIXTURE.md"),
        (sandbox_plan_docc, "TrustedLocalAdapterSandboxPreflightReportFixture"),
        (docs_index, "TRUSTED_LOCAL_ADAPTER_SANDBOX_PREFLIGHT_REPORT_FIXTURE.md"),
        (docc_root, "TrustedLocalAdapterSandboxPreflightReportFixture"),
        (capabilities, "TRUSTED_LOCAL_ADAPTER_SANDBOX_PREFLIGHT_REPORT_FIXTURE.md"),
        (capabilities_docc, "TrustedLocalAdapterSandboxPreflightReportFixture"),
        (roadmap, "TRUSTED_LOCAL_ADAPTER_SANDBOX_PREFLIGHT_REPORT_FIXTURE.md"),
        (roadmap_docc, "TrustedLocalAdapterSandboxPreflightReportFixture"),
    ):
        assert required in path.read_text(encoding="utf-8"), (
            f"Reference {required!r} not found in {path}"
        )
...
def test_trusted_local_adapter_run_request_fixture_is_documented() -> None:
    ...
    registry = ROOT / "tests" / "fixtures" / "repository_plugins" / "generic-registry.example.json"
    github_doc = ROOT / "docs" / "TRUSTED_LOCAL_ADAPTER_RUN_REQUEST_FIXTURE.md"
    docc_doc = (
        ROOT
        / "Sources"
        / "SpecHarvester"
    ...
    assert payload["apiVersion"] == "spec-harvester.trusted-local-adapter-run-request/v0"
    assert payload["kind"] == "SpecHarvesterTrustedLocalAdapterRunRequest"
    assert payload["schemaVersion"] == 1
    assert payload["authority"] == "producer_trusted_local_adapter_run_request_only"
    assert payload["contract"] == {
        "purpose": (
            "Record an explicit trusted local adapter run request for future "
            "preflight without granting execution authority."
        ),
        "contractVersion": "0.1.0",
        "requestAuthority": "producer_local_operator_request_only",
        "inputAuthority": "static_local_evidence_only",
    ...
    assert payload["operatorOptIn"] == {
        "required": True,
        "provided": True,
        "scope": "trusted_local_adapter_run_preflight_fixture_only",
        "operator": "local_operator_fixture",
        "acknowledgedBoundaries": [
            "request_requires_preflight_before_execution",
            "request_does_not_load_adapter_code",
            "request_does_not_run_adapter_processes",
            "request_does_not_grant_registry_authority",
        ],
    }
    assert payload["pathPolicy"] == {
        "pathFormat": "posix_relative",
    ...
    preflight_digest = f"sha256:{hashlib.sha256(preflight.read_bytes()).hexdigest()}"
    static_evidence_digest = f"sha256:{hashlib.sha256(static_evidence.read_bytes()).hexdigest()}"
    registry_digest = f"sha256:{hashlib.sha256(registry.read_bytes()).hexdigest()}"

    assert payload["adapterContractReferences"]["manifest"] == {
        "path": "tests/fixtures/repository_plugins/adapter-manifest.example.json",
        "digest": manifest_digest,
        "kind": "SpecHarvesterRepositoryPluginAdapterManifest",
        "authority": "producer_plugin_adapter_manifest_only",
    }
    assert payload["adapterContractReferences"]["preflightReport"] == {
        "path": "tests/fixtures/repository_plugins/adapter-preflight-report.example.json",
        "digest": preflight_digest,
        "kind": "SpecHarvesterRepositoryPluginAdapterPreflightReport",
        "authority": "producer_plugin_adapter_preflight_only",
    }
    assert payload["adapterContractReferences"]["allowedAdapterIds"] == [
        "spec_harvester.adapters.generic.parser_profile_summary.v0",
        "spec_harvester.adapters.generic.manifest_summary.v0",
        "spec_harvester.adapters.generic.package_topology_hint.v0",
    ]
    ...
    for record in payload["declaredInputArtifacts"]:
        assert_safe_relative_path(record["path"])
        assert record["digest"].startswith("sha256:")
        assert len(record["digest"]) == len("sha256:") + 64

    assert payload["readAllowlist"] == {
        "base": "repository_root",
        "policy": "declared_artifact_paths_only",
        "allowedPaths": [
            "tests/fixtures/repository_plugins/adapter-manifest.example.json",
            "tests/fixtures/repository_plugins/adapter-preflight-report.example.json",
    ...
    for path in payload["readAllowlist"]["allowedPaths"]:
        assert_safe_relative_path(path)

    output_policy = payload["outputPolicy"]
    assert output_policy["outputRoot"] == "artifacts/trusted-local-adapter-runs/example-workspace"
    assert output_policy["declaredOutputDirectoriesOnly"] is True
    assert output_policy["allowedOutputDirectories"] == [
        "artifacts/trusted-local-adapter-runs/example-workspace"
    ]
    assert output_policy["requireOutputDigests"] is True
    assert output_policy["overwritePolicy"] == "replace_single_run_directory_only"
    for path in output_policy["allowedOutputDirectories"]:
        assert_safe_relative_path(path)
    for record in output_policy["declaredOutputs"]:
        assert record["authority"].startswith("producer_trusted_local_adapter_")
        assert record["maxBytes"] <= 32768
        assert_safe_relative_path(record["path"])

    assert payload["resourceBudgets"] == {
        "timeoutMs": 30000,
        "adapterStartupTimeoutMs": 5000,
        "maxOutputBytes": 65536,
        "maxOutputFiles": 8,
        "maxDiagnostics": 20,
        "maxProcessCount": 0,
    }
    assert payload["environmentPolicy"] == {
        "inheritEnvironment": False,
        "allowedEnvironmentVariables": [],
        "deniedEnvironmentPatterns": ["*"],
        "secretsAccess": "not_allowed",
        "currentWorkingDirectory": "isolated_output_directory",
    }
    ...
    assert payload["dependencyPolicy"] == {
        "dependencyInstallation": "not_allowed",
        "useExistingRepositoryDependencies": "not_allowed",
        "packageManagers": "not_invoked",
        "packageManagerCommands": [],
    }
    assert payload["processPolicy"] == {
        "adapterProcessExecution": "not_allowed_in_request_fixture",
        "runnerMayValidateOnly": True,
        "unboundedProcessExecution": "not_allowed",
        "harvestedCodeExecution": "not_allowed",
        "aiExecution": "not_run",
    }
    assert payload["executionBoundary"] == {
    ...
    requested_adapters = {record["adapterId"]: record for record in payload["requestedAdapters"]}
    assert set(requested_adapters) == set(payload["adapterContractReferences"]["allowedAdapterIds"])
    for record in requested_adapters.values():
        assert record["requestedOutputs"]
        assert record["executionMode"] == "future_trusted_local_tool_after_preflight"
        assert record["currentExecution"] == "not_run"
        assert record["authority"] == "producer_adapter_output_only"

    required_non_authority = {
        "request_is_not_execution_permission",
        "does_not_load_third_party_adapter_code",
        "does_not_execute_adapters",
        "does_not_run_adapter_processes",
        "does_not_clone_or_fetch_repositories",
    ...
    assert required_non_authority.issubset(set(payload["nonAuthorityStatements"]))
    assert payload["followUp"] == {
        "trustedLocalAdapterRunPreflightTask": "P41-T3",
    ...
    for path in (github_doc, docc_doc):
        ...
        for required in (
            ...
            "producer_trusted_local_adapter_run_request_only",
            "producer_local_operator_request_only",
            "explicit operator opt-in",
            "adapter manifest",
            "adapter preflight report",
            "declared input artifacts",
    ...
    for path, required in (
        (docs_index, "TRUSTED_LOCAL_ADAPTER_RUN_REQUEST_FIXTURE.md"),
        (docc_root, "docs/TRUSTED_LOCAL_ADAPTER_RUN_REQUEST_FIXTURE.md"),
        (docc_root, "TrustedLocalAdapterRunRequestFixture"),
        (capabilities, "TRUSTED_LOCAL_ADAPTER_RUN_REQUEST_FIXTURE.md"),
        (capabilities_docc, "TrustedLocalAdapterRunRequestFixture"),
        (roadmap, "TRUSTED_LOCAL_ADAPTER_RUN_REQUEST_FIXTURE.md"),
        (roadmap_docc, "TrustedLocalAdapterRunRequestFixture"),
        (readiness_doc, "TRUSTED_LOCAL_ADAPTER_RUN_REQUEST_FIXTURE.md"),
        (readiness_docc, "TrustedLocalAdapterRunRequestFixture"),
    ):
        assert required in path.read_text(encoding="utf-8"), (
            f"Reference {required!r} not found in {path}"
        )
...
def test_trusted_local_adapter_runner_skeleton_is_documented() -> None:
    request = (
        ROOT
        / "tests"
        / "fixtures"
    ...
    github_doc = ROOT / "docs" / "TRUSTED_LOCAL_ADAPTER_RUNNER_SKELETON.md"
    docc_doc = (
        ROOT
        / "Sources"
        / "SpecHarvester"
        / "Documentation.docc"
        / "TrustedLocalAdapterRunnerSkeleton.md"
    )
    request_doc = ROOT / "docs" / "TRUSTED_LOCAL_ADAPTER_RUN_REQUEST_FIXTURE.md"
    request_docc = (
        ROOT
    ...
    preflight_doc = ROOT / "docs" / "TRUSTED_LOCAL_ADAPTER_RUN_PREFLIGHT_REPORT_FIXTURE.md"
    preflight_docc = (
        ROOT
        / "Sources"
        / "SpecHarvester"
    ...
    assert report["contract"]["runnerReportIsExecutionPermission"] is False
    assert report["runner"]["mode"] == "disabled_no_execution_skeleton"
    assert report["runner"]["enabled"] is False
    assert report["runner"]["adapterExecution"] == "not_run"
    assert report["runner"]["adapterCodeLoaded"] is False
    assert report["runner"]["adapterCodeImportAttempted"] is False
    assert report["runner"]["adapterProcessSpawned"] is False
    assert report["runner"]["executedAdapterCount"] == 0
    assert report["runner"]["dependencyInstallation"] == "not_allowed"
    assert report["runner"]["packageManagers"] == "not_invoked"
    assert report["runner"]["harvestedCodeExecution"] == "not_allowed"
    assert report["runner"]["aiExecution"] == "not_run"
    assert report["runner"]["networkAccess"] == "none"
    assert report["executionBoundary"] == {
        "adapterExecution": "not_run",
        "adapterCodeLoaded": False,
    ...
    checks = {record["code"]: record for record in report["validation"]["acceptedChecks"]}
    assert set(checks) == {
        "request_identity_valid",
        "request_boundary_no_execution",
        "preflight_identity_valid",
        "preflight_status_passed_review_only",
        "preflight_request_digest_matches",
        "runner_disabled_no_execution",
        "non_authority_boundary_preserved",
    }
    assert all(record["status"] == "passed" for record in checks.values())
    ...
    assert required_non_authority.issubset(set(report["nonAuthorityStatements"]))
    ...
    for path in (github_doc, docc_doc):
        ...
        for required in (
            ...
            "SpecHarvesterTrustedLocalAdapterRunRequest",
            "SpecHarvesterTrustedLocalAdapterRunPreflightReport",
            "spec-harvester.trusted-local-adapter-run/v0",
            "producer_trusted_local_adapter_run_report_only",
            "disabled by default",
            "no execution mode",
            "deterministic no-execution report",
            "request digest",
            "request identity failure",
            "preflight identity failure",
            "non-passing preflight status",
    ...
    for path, required in (
        (docs_index, "TRUSTED_LOCAL_ADAPTER_RUNNER_SKELETON.md"),
        (docc_root, "docs/TRUSTED_LOCAL_ADAPTER_RUNNER_SKELETON.md"),
        (docc_root, "TrustedLocalAdapterRunnerSkeleton"),
        (capabilities, "TRUSTED_LOCAL_ADAPTER_RUNNER_SKELETON.md"),
        (capabilities_docc, "TrustedLocalAdapterRunnerSkeleton"),
        (roadmap, "TRUSTED_LOCAL_ADAPTER_RUNNER_SKELETON.md"),
        (roadmap_docc, "TrustedLocalAdapterRunnerSkeleton"),
        (readiness_doc, "TRUSTED_LOCAL_ADAPTER_RUNNER_SKELETON.md"),
        (readiness_docc, "TrustedLocalAdapterRunnerSkeleton"),
        (request_doc, "TRUSTED_LOCAL_ADAPTER_RUNNER_SKELETON.md"),
        (request_docc, "TrustedLocalAdapterRunnerSkeleton"),
        (preflight_doc, "TRUSTED_LOCAL_ADAPTER_RUNNER_SKELETON.md"),
        (preflight_docc, "TrustedLocalAdapterRunnerSkeleton"),
    ):
        assert required in path.read_text(encoding="utf-8"), (
            f"Reference {required!r} not found in {path}"
        )
