...
def load_local_candidate_review_catalog(path: Path) -> dict[str, Any]:
    ...
    if (
        catalog.get("apiVersion") != CATALOG_API_VERSION
        or catalog.get("kind") != CATALOG_KIND
        or catalog.get("authority") != CATALOG_AUTHORITY
    ):
