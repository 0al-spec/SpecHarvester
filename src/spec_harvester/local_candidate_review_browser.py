...
function render(){const rows=filtered();state.cursor=Math.min(state.cursor,Math.max(rows.length-1,0));const body=document.getElementById('results');body.replaceChildren();rows.forEach((item,index)=>{const tr=document.createElement('tr');if(index===state.cursor)tr.className='selected';['candidateId','readiness','warningCount','corrected','ecosystem','packageShape','preflightStatus','reviewState'].forEach(key=>{const td=document.createElement('td');const span=document.createElement('span');span.className='badge';setText(span,item[key]);td.append(span);tr.append(td)});tr.addEventListener('click',()=>{state.cursor=index;save();render()});body.append(tr)});renderDetail(rows[state.cursor]);setText(document.getElementById('queue-position'),rows.length?`Queue ${state.cursor+1} of ${rows.length}`:'No matching candidates');document.getElementById('previous').disabled=!rows.length||state.cursor===0;document.getElementById('next').disabled=!rows.length||state.cursor>=rows.length-1;save(rows)}
