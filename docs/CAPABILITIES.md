...
| Local candidate review browser | Renders a local candidate-only static browser with corpus summary, facets, search, sort, and resumable queue state. | [`LOCAL_CANDIDATE_REVIEW_BROWSER.md`](LOCAL_CANDIDATE_REVIEW_BROWSER.md) |
...
| Local review decision service | Stores schema-valid catalog-bound decisions behind a loopback Origin/CSRF boundary with four validated reviewer actions, atomic replacement history, progress summaries, and portable evidence-only import/export. | [`LOCAL_REVIEW_DECISION_SERVICE.md`](LOCAL_REVIEW_DECISION_SERVICE.md) |
