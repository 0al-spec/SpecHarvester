...
| Trusted local adapter runtime readiness | Phase 41 plans the safe path from adapter contracts toward future trusted local execution: explicit operator opt-in, run request fixtures, preflight fixtures, disabled no-execution runner skeleton, review-only batch evidence handoff, and real local readiness validation. It does not enable adapter execution. | [`TRUSTED_LOCAL_ADAPTER_RUNTIME_READINESS.md`](TRUSTED_LOCAL_ADAPTER_RUNTIME_READINESS.md), [`REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md`](REPOSITORY_PLUGIN_ADAPTER_EXECUTION_POLICY.md), [`REPOSITORY_PLUGIN_ADAPTER_REAL_LOCAL_VALIDATION.md`](REPOSITORY_PLUGIN_ADAPTER_REAL_LOCAL_VALIDATION.md) |
