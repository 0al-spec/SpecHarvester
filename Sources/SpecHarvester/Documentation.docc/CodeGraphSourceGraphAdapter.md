...
The normalized payload records trust policy, source target metadata, input and
executable digests, source file digests, and bounded file, node, edge, and
diagnostic records.

## Trust Boundary

Every payload declares `trustLevel: untrusted_optional_tool`,
`classification: third_party_local_binary`, `executedRepositoryCode: false`,
`allowedNetwork: false`, and `installation: out_of_band_required`.
