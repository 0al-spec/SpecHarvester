...
| Repository plugin adapter contract | Phase 40 defines the language- and framework-agnostic boundary for future adapter manifests, preflight reports, execution policy, sandbox expectations, and review-only adapter output evidence while keeping static applicability as the default safe path. | <doc:RepositoryPluginAdapterContract>, <doc:RepositoryPluginSubsystemContract>, <doc:StaticRepositoryPluginApplicabilityEvaluator> |
