...
| Trusted local adapter runtime readiness | Phase 41 plans the safe path from adapter contracts toward future trusted local execution: explicit operator opt-in, run request fixtures, preflight fixtures, disabled no-execution runner skeleton, review-only batch evidence handoff, and real local readiness validation. It does not enable adapter execution. | <doc:TrustedLocalAdapterRuntimeReadiness>, <doc:RepositoryPluginAdapterExecutionPolicy>, <doc:RepositoryPluginAdapterRealLocalValidation> |
