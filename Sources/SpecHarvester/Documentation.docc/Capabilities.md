...
| Repository plugin adapter contract | Phase 40 defines the language- and framework-agnostic boundary for future adapter manifests, preflight reports, execution policy, sandbox expectations, and review-only adapter output evidence while keeping static applicability as the default safe path. P40-T2 adds the first machine-readable `SpecHarvesterRepositoryPluginAdapterManifest` fixture, and P40-T3 adds the first `SpecHarvesterRepositoryPluginAdapterPreflightReport` fixture for allowed, rejected, fallback, and blocked decisions before adapter code can run. | <doc:RepositoryPluginAdapterContract>, <doc:RepositoryPluginAdapterManifestFixture>, <doc:RepositoryPluginAdapterPreflightReportFixture>, <doc:RepositoryPluginSubsystemContract>, <doc:StaticRepositoryPluginApplicabilityEvaluator> |
