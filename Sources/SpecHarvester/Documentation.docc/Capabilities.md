...
| Operational MVP validation | Phase 43 validates whether the current pipeline produces valid, repository-specific starter packages over a bounded pinned multi-ecosystem corpus before broader autonomous scraping or real adapter execution. P43-T2 adds the machine-readable `SpecHarvesterOperationalMVPValidationPlan` fixture; P43-T3 adds the companion `SpecHarvesterOperationalMVPValidationReport` fixture for per-repository result, quality, stop-policy, and handoff readiness fields; P43-T4 records the real local static-only baseline over xyflow, FastAPI, and Gin pinned checkouts; P43-T5 records the live local LM Studio AI-enabled comparison while keeping all AI draft/enrichment output proposal-only; P43-T6 adds author handoff summaries for valid, reviewable, manual-correction, and do-not-promote outcomes; P43-T7 selects `needs_quality_hardening` before broader autonomous popular-library scraping; P44-T1 triages the shared `package_set_id_missing` AI draft warning as an AI proposal shape issue before later quality-hardening work. | <doc:OperationalMVPValidationPlan>, <doc:OperationalMVPValidationPlanFixture>, <doc:OperationalMVPValidationReportFixture>, <doc:OperationalMVPStaticOnlyBaseline>, <doc:OperationalMVPAIEnabledComparison>, <doc:OperationalMVPAuthorHandoffSummaries>, <doc:OperationalMVPExitReport>, <doc:OperationalMVPWarningTriage>, <doc:AuthorReadyDraftQualityReport>, <doc:AutonomousCandidateBatch> |
