...
The manifest authority means the record is a declared producer-side adapter
contract, not adapter preflight, not adapter execution permission, not accepted
SpecPM package truth, and not registry authority.

## Relationship to Static Evidence

The fixture references the existing static evidence envelope and generic plugin
registry fixtures:
...
Each `adapters[]` item declares `adapterId`, `contractVersion`,
`adapterVersion`, `roles[]`, `title`, `summary`, `authority`,
`inputEvidence`, `outputs[]`, `execution`, `sandboxRequirements`,
`capabilityRequests`, `diagnostics[]`, and `nonAuthorityStatements[]`.
...
Every example adapter uses `mode: static_only`, `defaultEnabled: false`,
...
Every adapter record must declare sandbox requirements: `readPathPolicy`,
`writePathPolicy`, `networkAccess`, `dependencyInstallation`,
`packageManagers`, `harvestedCodeExecution`, `processExecution`,
`environmentAccess`, `timeoutMs`, and `maxOutputBytes`.
...
evidence only. The fixture declares `adapter_parser_profile_summary`,
`adapter_manifest_summary`, `adapter_topology_hint`,
`adapter_review_panel_data`, and `adapter_diagnostics`.
