...
<doc:XyflowPackageSetIdentityRegenerationDryRun>. That artifact can feed a
future refreshed selected handoff, but it remains producer preview evidence
and not SpecPM acceptance.
P32-T4 records the single-package deferred regeneration result in
<doc:SinglePackageDeferredCandidateRegenerationDryRun>. That artifact keeps
`cupertino.core` deferred and makes `navigation_split_view.core` eligible for
refreshed candidate-layer review, while still remaining producer preview
evidence only.
