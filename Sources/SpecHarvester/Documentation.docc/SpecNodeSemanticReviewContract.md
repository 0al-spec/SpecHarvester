...
This page mirrors the GitHub clean-context semantic review contract for
...
It emits typed findings. It cannot mutate candidates, emit patch operations,
apply retry directives, or override structural validation.
...
`SpecNodeSemanticReviewContract` complements
<doc:SpecNodePatchProposalContract> and <doc:SpecNodeRefinementPromptContract>.
...
The job must exclude `firstPassPromptTranscript: excluded`,
`chainOfThought: excluded`, `firstPassProviderLogs: excluded`,
`providerLogs: excluded`, `rawRepositorySource: excluded`,
`documentationBodies: excluded`, `dependencyDirectories: excluded`,
`secrets: excluded`, `arbitraryPrompts: excluded`, and
...
Allowed verdicts are `approve`, `needs_revision`, and `reject`. Allowed
...
`prompt_contract_violation` covers task self-description such as
`generate_specpm`.
...
Allowed references include artifact IDs such as `harvest_snapshot`,
`spec_package_manifest`, `boundary_spec`, and `public_interface_index`;
...
It must not contain `candidatePatchProposal`, `proposal`, `operations`, `patch`,
`patches`, `retryDirective`, `retryDirectives`, `rawUnifiedDiff`,
`fullFileReplacement`, `shellCommand`, `gitCommand`, `networkFetch`,
`providerCall`, `packageManagerCommand`, `testRunnerCommand`,
...
SpecHarvester must reject semantic review output when output is not
`SpecNodeSemanticReviewResult`, schema version is unsupported, job digest or
reviewed refinement result digest does not match, verdict is outside the
allowed set, findings are missing for `needs_revision` or `reject`, `approve`
contains blocking findings, `reject` lacks a blocking finding, any finding code
or severity is unknown, any finding lacks a non-empty message or known evidence
reference, or output contains mutation, patch, retry, shell, network, or direct
file-write fields.
